...
public class MenuAndDesktopEntryHandler {
    ...
    private final static Logger LOG = LoggerFactory.getLogger(MenuAndDesktopEntryHandler.class);
    ...
    public void addEntries(final MenuAndDesktopEntriesFactory factory, final JNLPFile jnlpFile, final boolean addMenu, final boolean addDesktop) {
    ...
    public void updateEntries(final MenuAndDesktopEntriesFactory factory, final JNLPFile jnlpFile, final boolean updateMenu, final boolean updateDesktop) {
        Executors.newSingleThreadExecutor().execute(() -> {
            if (updateMenu) {
                try {
    ...
    public void askForIntegration(final MenuAndDesktopEntriesFactory factory, final JNLPFile jnlpFile) {
        ...
        final ShortcutCreationOptions shortcutCreationOptions = ShortcutCreationOptions.forConfigName(property).orElse(ShortcutCreationOptions.CREATE_NEVER);

        if (shortcutCreationOptions == ShortcutCreationOptions.CREATE_NEVER) {
            return;
        }
        ...
        if (hasMenu || hasDesktop) {
            ...
            if (shortcutCreationOptions == ShortcutCreationOptions.CREATE_ALWAYS) {
                addEntries(factory, jnlpFile, supportsMenu, supportsDesktop);
                return;
            }
            if (shortcutCreationOptions == ShortcutCreationOptions.CREATE_ALWAYS_IF_HINTED) {
                addEntries(factory, jnlpFile, supportsMenu && jnlpWantsMenu, supportsDesktop && jnlpWantsDesktop);
                return;
            }
            if (shortcutCreationOptions == ShortcutCreationOptions.CREATE_ASK_USER) {
            ...
            if (shortcutCreationOptions == ShortcutCreationOptions.CREATE_ASK_USER_IF_HINTED) {
                if(jnlpWantsDesktop || jnlpWantsMenu) {
