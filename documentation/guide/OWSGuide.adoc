...
== Installation
OpenWebStart can be installed on Windows, macOS and Linux operating systems. There are two different ways to install OpenWebStart:
...
1. Go to the directory where the installer (DEB file) is stored and run the file from the terminal +
   `sudo dpkg -i OpenWebStart_linux_1_x_x.deb`
...
If you specify an absolute path, e.g. `sys.installationDir=c:\test`, then OWS will be installed there.

If you specify a relative path, e.g. `sys.installationDir=test`, then OWS will be installed in the specified directory relative to the directory where the unattended installer command is executed.
...
=== Running OWS from a portable USB stick
...
So if you can (and want to) do without the file associations, desktop icon, and the startup menu, you may create a portable version by yourself.
...
especially when one installation is for the _current user_ (i.e. without admin privileges) and the other one is for _all users,
...
It is possible to define an update strategy while opening the OWS Settings and while launching of a Jnlp application on
every `start`, `daily`, `weekly`, `monthly`, or `never`.
...
If a property is locked, then later configurations may not modify the value.
...
After adjusting and saving as you prefer, the configuration with the modified properties is written to the local `deployment.properties` file as described above.
...
Special characters need an escape character:
...
On some systems OpenWebStart is used beside Oracle's web start.
...
The content of the file is the same as described above. OpenWebStart will pick up
...
* jvm_cache: directory where a common set of downloaded JVMs are stored (this can be separately configured using the `ows.jvm.manager.cache.dir` property)
...
The available JVMs are listed. One can activate/deactivate/remove the JVMs that OWS will consider for launching Jnlp Applications.
One can choose or automatically add locally available JVMs. Moreover, one can specify in the `Settings` the server from where JVMs can be downloaded:

image::OWS_jvm_config.png[title="Configuring JVM Management" width="80%"]

==== JVM update strategy

OWS can update the JVM being used to launch an application based on the chosen update strategy:

NO_REMOTE (never check the JVM update server to download a new JVM)

DO_NOTHING_ON_LOCAL_MATCH (do not check for updates the JVM update server on local match but do download if no local match found)

ASK_FOR_UPDATE_ON_LOCAL_MATCH (check for update on local match but ask before installing new JVM) – this is the default option

AUTOMATICALLY_DOWNLOAD (check and install the newest JVM)
...
cacheTimeInMillis:: The time that needs to elapse before a client is allowed to contact the server again. Usually the server is accessed once per application startup.

os:: Possible values are: MAC64, MAC32, MACARM64, LINUX64, LINUX32, WIN64, WIN32
...
There are a few settings that can be used to configure the search for local JVMs.
...
===== Choosing the JVM version
OWS starts the JNLP application with the JVM that best matches the JVM specified in the JNLP file.
...
===== Specify a preferred JVM vendor in the JNLP file

As illustrated above in figure 4, you can specify a particular vendor that should be taken into consideration for selecting the JVM that launches your JNLP application.
...
===== Specify 32-bit JVM in the JNLP file

There is the possibility to advise OpenWebStart to use the 32-bit JVM on a 64-bit machine by specifying the `require-32bit` attribute in the JNLP file as follows:
...
Moreover, using the environment variable `JAVAWS_VM_ARGS`, you can specify JVM arguments that will be applied to all JNLP files in addition to those specified in a JNLP file:

[source]
----
set JAVAWS_VM_ARGS=-Xss2048k -Xmx4096m
----
...
* Java Swing high dpi properties jvm arguments
...
Subsequently, the JNLP file and the environment variable `JAVAWS_VM_ARGS` can include the whitelisted JVM args:
...
===== JNLP file-specific JVM Arguments
...
To facilitate caching of resources by OWS it is necessary, that the server from where the resources are downloaded is configured to respond to _HTTP HEAD_ request.
...
* to delete the application from cache

=== Proxy Settings

It is possible to configure a proxy to be used by OWS when it downloads jnlp files and resources.
OWS will use these settings to set up a proxy with the java.net package.
...
The _System Proxy_ option is trying to imitate the behavior of your operating system (OS) and the proxy settings which are defined there.
E.g., for Windows the settings are read from the registry and then converted into a java.net proxy.
macOS and Linux are working similarly.
...
You can specify a PAC proxy. A PAC file is a small JavaScript file that defines rules for choosing the appropriate
proxy server for a given URL or request.

OWS uses Mozilla Rhino, an open‑source JavaScript engine, to run the PAC JavaScript.
Sometimes, while processing a complex PAC script, the Rhino Engine may run out of its internal stack memory.
This can be avoided by specifying a JVM argument `-Xss`, e.g. `-Xss2048k`, for the JVM used in Stages 1 and 2 of OWS.
For the Stage 1 JVM, specify the JVM argument in the javaws.vmoptions file in the OWS Install directory
For the Stage 2 JVM, specify the JVM argument in either the JNLP file, or the `JAVAWS_VM_ARGS` environment variable, or
in the `deployment.properties` file using property for JNLP specific JVM arguments `ows.jvmargs.for`.
...
While the bundled JVM cannot be customized or replaced by an OpenWebStart user, the JVM used to run the JNLP application is determined by the definition in the JNLP file and by the configuration of the OpenWebStart JVM Manager.
...
|https://subdomain.domain.com|https://subdomain.domain.com:443|since HTTPS is used, the default port is 443
...
|\*.domain.com:*|https://\*.domain.com:*|any domain that ends in ".domain.com" and any port is whitelisted
...
|https://*jvms.domain.com:443|Error: invalid host|for the host part use either * or text but not combination
...
The main log file can easily be determined by its size as it contains more log lines.
...
Note that the log file of the OpenWebStart Settings application also goes to this log directory. They are named
...
OWS specifying when and how desktop and start menu shortcuts are created.
...
* _Ask if hinted_: Ask user if hinted in the jnlp file
* _Always if hinted_: Always create when hinted in the jnlp file
...
OWS can be started from the command line as follows:
...
the jar files for the app are available in the cache. For example:
...
|X
|X
|Allows specifying the directory where the JVM cache is located. The follow example shows two examples for Windows: ows.jvm.manager.cache.dir=c:\\temp\\JVMCacheDir or ows.jvm.manager.cache.dir=c\:/temp/JVMCacheDir

|ows.jvm.manager.cache.cleanup
|X
|X
|Allows disabling the cleanup of the local JVMs. Default is _true_. The benefit of disabling this is to avoid race conditions if two instances of OWS are launched at exactly the same time.
...
|X
|X
|Defines if a vendor attribute in a java/j2se tag of the JNLP file should be respected. If a specific JVM vendor is defined, this is false i.e. the vendor from the settings has precedence by default. If "Any Vendor" is selected in OWS Settings, this property is _true_ to take the vendor hint of the JNLP file into consideration.
...
ASK_FOR_UPDATE_ON_LOCAL_MATCH (check for update on local match but ask before installing new JVM) - the default value
...
|X
|X
|If set to _true_, OpenWebStart will search for new local JVMs at every start. Default is _false_.

|ows.jvm.manager.excludeDefaultSearchLocation
|X
|X
|If set to _true_ OpenWebStart will exclude the default locations when searching for JVMs. Default is _false_.
...
|X
|X
|All local hosts should be bypassed. Default is _false_.
...
|X
|X
|If _true_, use the same web server and port for https and ftp as is configured for http. (This is only valid if deployment.proxy.type = 1 (manual proxy). Default is _false_.
...
|X
|X
|If set to _true_, http urls are not converted to https. Default is _false_.
...
|-
|X
|If set to _true_, debug logging is enabled. Default is _false_.

|deployment.log.file
|-
|X
|If set to _true_, log is sent to a file. Default is _false_.

|deployment.log.jnlpFileContent
|-
|X
|If set to _true_, log the contents of Jnlp file. Default is _false_.
...
|X
|X
|Defines how often OpenWebStart should search for updates when launching an application. Allowed values are ON_EVERY_START, DAILY, WEEKLY, MONTHLY, and NEVER.

|deployment.connection.connectTimeout
|
|X
|HTTP request connect timeout in milliseconds. Default is 10000 ms (10 s). 0 means infinite wait.

|deployment.connection.readTimeout
|
|X
|HTTP request read timeout in milliseconds. Default is 10000 ms (10 s). 0 means infinite wait.
...
_deployment.user.logdir=\#USER_HOME_DIR#\\.config\\ows\\log_

|deployment.security.itw.disablerestrictedfiles
|
|X
|Create regular files instead of restricted files if _true_. Default is _false_.

|deployment.log.console.clientapp
|
|X
|Log client application's output to console if _true_. Default is _false_.
If _true_, it may lead to memory leak in the console if the application outputs large volume of text.

|deployment.nosecurity
|
|X
|Disable security in OWS. Default is _false_, i.e., security IS enabled by default.
If _true_, then _java.lang.SecurityManger_ and _java.security.Policy_ are NOT set by OWS.
