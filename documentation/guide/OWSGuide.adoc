...
===== Specify a specific  JVM vendor in the JNLP file
...
|X
|X
|Allows to disable the cleanup of the local JVMs. Default is _true_. The benefit of disabling this is to avoid race conditions if two instances of OWS are launched at exactly the same time.
...
|-
|X
|If set to _true_ log is outputted to file. Default is _false_.
...
|
|X
|Create regular files instead of restricted files if _true_. Default if _false_.

|deployment.log.console.clientapp
|
|X
|Log client application''s outputs to console if _true_. Default if _false_.
If _true_, may lead to memory leak in console if the application output's large volume of text.

|deployment.nosecurity
|
|X
|Disable security in OWS. Default is _false_, i.e., security is enabled by default.
If _true_ then _java.lang.SecurityManger_ and _java.secutity.Policy_ are NOT set by OWS.
