...
For some specific configurations manually editing this file might be necessary, but for most cases the manipulation via OWS Settings application is sufficient.
...
The location of such an optional system-level `deployment.properties` file is determined by the System Administrator.
As described in the lifecycle above, if the `deployment.config` file exists, it is located in one of the directories shown in the following table:
...
The following system properties can be set to configure the location of the system configuration file:
...
The `jvm_cache` location can be configured using the property `ows.jvm.manager.cache.dir`  in the centralized
