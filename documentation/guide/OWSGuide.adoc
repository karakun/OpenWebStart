...
desktop intergation. So if you can (and want to) do without the file associations, desktop icon and startup menu
...
So you might want to update these files in the USB stick if you want to keep your OWS installation
up-to-date.
...
.Configuring OWS Settings
image::OWS_Configuration.png[width="80%", align="center"]
...
User level configuration can be specified or customized by manipulating `deployment.properties`.

=== System level configuration
There exists also the possibility to define a system-wide configuration.
This allows setting up a common configuration for multiple users on a single computer or helps in managing a corporate
infrastructure where many computers need to be configured identically.

In a centralized environment like the Citrix Server that Daimler has it makes sense to centralize certain things:

    deployment.properties : all users must use the same deployment.properties for OWS
    jvm_cache : all users should be using common set of jvms. Users should not be having their own jvms
    Security (certificate stores)

At the same time it does not make sense to centralize certain things becasue they are too user specific

    logs - logs for the app started by each user
    application cache and recently_used file - for the apps started by each user
    temp dir : OWS creates temp files. If this dir is centralized then there will be a clash between users
    icons - for the app started by user
    user decisions  (.appletTrustSettings)

At present centralized location for config and cache can be specified using XDG_CACHE_HOME and XDG_CONFIG_HOME env vars.

Under XDG_CONFIG_HOME location we are storing user specific things : logs, icons, user decisions.

Under XDG_CACHE_HOME location we are storing user specific things like app cache and recently_used file

I discovered that jvm_cache location is already configurable.

We already have a property :

deployment.user.jvmcachedir=c\:/temp/JVMCache

For centralized JVM Cache we can specify the above property in the centralized deployment.properties files which is pointed to by the centralized config file C:\Windows\Sun\Java\Deployment\deployment.config

Note: deployment.user.jvmcachedir spec in deployment.properties takes precedence over XDG_CACHE_HOME i.e. when both deployment.user.jvmcachedir and XDG_CACHE_HOME are specified then the jvm_cache will be created under the location specified by deployment.user.jvmcachedir.
The property is now defined as ows.jvm.manager.cache.dir

On Windows it can be specified in two ways as:
...
On Windows it is possible to have a common deployment.properties.

Create a deployment.config file (see attached) in C:\Windows\Sun\Java\Deployment directory. This file contains a property that points to the deployment.properties file, for example:

deployment.system.config=file:///c:/OpenWebStart/my.properties

2. OWS will then pick up deployment.properties from the specified file when deployment.properties is NOT present in <User Home>\.config\icedtea-web dir

3. When deployment.properties IS present in <User Home>\.config\icedtea-web dir then it seems to have precedence for specified properties.
...
// TODO
// - describe the option to turn on this experimental feature
...
#<documentation added in the near future>#
// TODO Janak
// have a look at the tooltips provided with the ows-settings
