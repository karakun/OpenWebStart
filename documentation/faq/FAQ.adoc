...
There is no need to have any JVM installed on your system to run OWS.
OWS has a JVM Manager which can find local JREs or download JREs from the Internet.

Don't worry if no JVMs are shown in JVM Manager after install.
If you want you can use _Find local_ or _Add local..._ to detect and add an existing Java instance on your client device or simply start the JNLP file and the JVM Manager will download an appropriate JRE for your application.
...
Only the JVMs manged by the JVM Manager can be used to run JNLP applications.
