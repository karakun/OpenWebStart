...
The second one is determined by the definition in the JNLP file or by the configuration via OpenWebStart JVM Manager.

With its half-yearly releases (spring and fall) we update the bundled JVM and thus the certificates included in the internal JVM.

During the execution of the JNLP application the certificates of the custom-selected JVM are available.
So the control which certificates will be available depends on the definitions in the JNLP file and the configuration via OpenWebStart JVM Manager.
