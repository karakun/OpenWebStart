...
The OWS truststores in the directory `<UserHome>/.config/icedtea-web/security` are protected by the default password "changeit" which is same as that used for the JVMs trustores.
OWS uses the default password (null) for the load (read) operation and the password ("changeit") for the store (write) operations on the truststores. However, it is possible that a user may have changed the password to ensure the integrity of the truststore.
In this case, OWS will prompt the user to enter the password for the truststore when a user tries to import a certificate in itw-settings or when a user selects "always trust" while running OWS.
