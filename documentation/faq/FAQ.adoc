...
OpenWebStart is released under the GNU GPL2 with Classpath Exception license.

== Installation

==== Do I have to install a Java JVM on my system to run OWS?
...
The downloaded JREs are stored in %userhome%/.cache/icedtea-web/jvm-cache
...
Your JNLP applications can be run with a  long-term support (LTS) version of Java such as 8, 11, 17, or 21.
We have not run any tests with higher versions of Java yet,
but from the experience collected while adding support for Java 11, 17, 21 we do not expect any big obstacles.
...
However, a custom installer can be provided on demand. In case you need such support, please write to openwebstart@karakun.com.
...
When JWS is installed, the `.jnlp` MIME type might have associated with it already and so OWS, when installed subsequently, may not be associated with it.
...
Please see the section `OWS specific System Configuration` in the OWS Guide.
...
Information on "how to view hidden folder and files" can be found at https://www.howtogeek.com/194671/how-to-hide-files-and-folders-on-every-operating-system/[this external link].
...
Sometimes OpenWebStart signals that the application's digital signature cannot be verified when launching an application with signed jars.
...
==== Password protection for the truststores

The OWS truststores in the directory `<UserHome>/.config/icedtea-web/security` are protected by the default password "changeit"
which is same as that used for the JVMs trustores.
OWS uses the default password ("changeit") for the load (read) operation and the store (write) operations on the truststores.
However, it is possible that a user may have changed the password to ensure the integrity of the truststore. In such a case, OWS will prompt the
user to enter the password for the truststore. The prompt dialog will be displayed while loading the truststore and also while storing to a truststore
when a user tries to remove or import a certificate in itw-settings or selects "always trust the certificate" in the jar certificate verification
warning while running OWS.

image::OWS_truststore_password_prompt.png[title="Prompt for Truststore Password" width="80%"]
...
Applets are not supported, and there are no plans to support them in the future. Almost all browsers have stopped support for the Java Plugin and hence the applets.
...
==== Is there a way to advise OpenWebstart to use a 32-bit JVM on a 64-bit machine?
...
Karakun maintains OpenWebStart as an open‑source project for the benefit of the wider community. Keeping the software compatible with newer versions of Java requires
continuous effort and resources. We therefore warmly encourage community members and organizations to support OWS — whether through contributions, sponsorship, paid support -
so that OWS can continue to thrive and evolve for everyone's benefit.
...
With this option you may report bugs using a dedicated communication channel on the https://board.karakun.com[support forum] and your bug reports will have a higher priority.
...
With premium support provided by Karakun you may report bugs or issues using the premium communication channel on the (via https://board.karakun.com[support forum]).
...
Please contact openwebstart@karakun.com to find out which support model fits your needs best.
